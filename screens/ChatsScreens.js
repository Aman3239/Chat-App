...
const ChatsScreen = () => {
  ...
  const getrequests = async () => {
    try {
      const response = await axios.get(`http://10.0.2.2:8000/getrequests/${userId}`)
  ...
  const accecptRequest = async requestId => {
    try {
      ...
      <View
        ...
        <Pressable onPress={logout}>
          <Image
            ...
            source={{
              uri: 'https://lh3.googleusercontent.com/ogw/AF2bZyjAblSUK2UCzLcJccIxE4lN23n1uZK6jkze9M3KZsnyNwE=s32-c-mo',
            }}
          />
        </Pressable>
              ...
              {requests?.map((item, index) => (
                <Pressable style={{ marginVertical: 12 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    ...
                    <Pressable onPress={() => accecptRequest(item?.from?._id)} style={{ padding: 8, backgroundColor: "#005187", width: 75, borderRadius: 5 }}>
                      <Text style={{ fontSize: 13, textAlign: "center", color: "white" }}>Accept</Text>
                    </Pressable>
